...
Ext.define("PSO2.AbilitySet", {
    ...
    constructor: function(abilitySet) {
        var abilitySet = this,
            affixEntry;
        Ext.apply(abilitySet, abilitySet);
		// Find id of Mutation I, Mutation II, and Photon Collect for boost upgrade calculations
        if (abilitySet.abilityStore) {
            affixEntry = abilitySet.abilityStore.findRecord("name", "Mutation I");
            if (affixEntry) {
                abilitySet.mutationICd = affixEntry.get("code")
            }
            affixEntry = abilitySet.abilityStore.findRecord("name", "Mutation II");
            if (affixEntry) {
                abilitySet.mutationIICd = affixEntry.get("code")
            }
            affixEntry = abilitySet.abilityStore.findRecord("name", "Photon Collect");
            if (affixEntry) {
                abilitySet.photonCd = affixEntry.get("code")
            }
        }
        abilitySet.stores = [];
        abilitySet.clear()
    ...
    put: function(affix) {
        var abilitySet = this,
            c, b = affix;
        if (!abilitySet.hashStack[affix.code]) {
            abilitySet.hashStack[affix.code] = 0
        }
        c = ++abilitySet.hashStack[affix.code];
        if (Ext.isArray(affix.extend)) {
            if (0 < affix.extend[abilitySet.overflow(affix.extend.length, c, 1)] && Ext.Array.indexOf(d.stack, affix) < 0) {
                abilitySet.stack.push(affix)
            }
        }
    },
    ...
    resetAbility: function() {
        var abilitySet = this,
            numFod = abilitySet.stores.length,
            minAffix = abilitySet.getEnableData(0).length - abilitySet.getFactorCount(0),
            b;
        if (minAffix == 0) {
            b = false;
            for (i = 1; i < numFod; i++) {
        ...
        if (b == true) {
            ...
            abilitySet.enableCheckMax = minAffix + 1;
    ...
    resetExtendAbility: function() {
        ...
        for (affixCode in abilitySet.hashStack) {
        ...
        for (index = 0; index < baseLength; index++) {
            if (base[index].indexOf("*") < 0) {
                ...
                var regex = new RegExp("(" + base[index].replace("*", "[^,]+") + ")", "g"),
                    relTarget = abilitySet.getKeyList(hashStack).join(",").match(regex);
                if (relTarget) {
                    g = relTarget
                } else {
                    return null
...
Ext.define("PSO2.AbilityComponent", {
            ...
            fodderSlot = Ext.create("Ext.data.Store", {
                ...
                chkFn: function(affix1, affix2) {
                    var excludePatterns = abilityComponent.excludePattern,
                        i, j, excludePattern, match1, match2;
                    for (i = 0;ix < excludePatterns.length;ix++) {
                        excludePattern = Ext.isArray(excludePatterns[i]) ? excludePatterns[i] : [excludePatterns[i]];
                        match1 = match2 = false;
                        for (j = 0; j < excludePattern.length; j++) {
                ...
                getEnableData: function() {
                    var index, slotCount = this.getCount(),
                        result = [],
                        affixEntry;
                    for (index = 0; index < slotCount; index++) {
    ...
    getSuccessList2: function(abSet, affixList) {
        var abComp = this,
            ...
            abilityStore = abComp.getAbilityStore(),
            d = affixList.length,
            affixEntry, c, successRate, rateList = {};
        for (c = 0; c < d; c++) {
...
Ext.define("PSO2.ResultPanel", {
    ...
    initItems: function() {
        ...
        resultPanel.selOpt2 = resultPanel.createComboBox(resultPanel.constSelOption2, resultPanel.optionStore2, resultPanel.initOption2Value, "opt2change", function(ddMenu) {
            if (ddMenu.value == null || ddMenu.originalValue == ddMenu.value) {
                this.optionItems = []
            } else {
                this.optionItems = [this.getSelectOptionRecord(ddMenu)]
            }
        });
    ...
    createComboBox: function(ddMenuID, optStore, initValue, event, func) {
        var resultPanel = this;
        return Ext.create("Ext.form.field.ComboBox", {
            id: resultPanel.id + ddMenuID,
            store: optStore,
            displayField: "id",
            forceSelection: true,
            editable: false,
            ...
            listeners: {
                scope: resultPanel,
                change: function(ddMenu, menuItem) {
                    if (Ext.isFunction(func)) {
                        func.call(this, ddMenu)
                    }
                    if (menuItem !== true) {
                        this.refresh();
                        this.fireEvent(event, this, ddMenu, ddMenu.originalValue == ddMenu.value)
                    }
                }
            }
        })
    ...
    getSelectOptionRecord: function(ddMenu) {
        return ddMenu.findRecord("value", ddMenu.getValue())
    },
    // Set drop down menu to item
    selectOption: function(ddItem) {
        var resultPanel = this,
            ddMenu = resultPanel.prefixOptions[ddItem.charAt(0)];
        if (ddMenu) {
            ddMenu.select(ddItem)
        }
    },
    refresh: function() {
        var resultPanel = this,
    ...
    isExcludePattern: function(code1, code2) {
        ...
        for (var e = 0; e < exPatternLen; e++) {
            ...
            for (index = 0; index < excludePattern.length; index++) {
    ...
    addAbility: function(selAffix, c) {
        var resultPanel = this,
            removeList = [],
            resItemLen = resultPanel.resultItems.length,
            index;
        for (index = 0; index < resItemLen; index++) {
        ...
        for (index = 0; index < removeList.length; index++) {
    ...
    removeAbility: function(deselAffix, a) {
        var resultPanel = this,
    ...
    doDo: function(success, fail) {
        var resultPanel = this,
            ...
            index, affixRate;
        if (0 < selAffixLen) {
            for (index = 0; index < selAffixLen; index++) {
    ...
    getSuccessPattern: function(numFail, rateList, itemBoostFn) {
        var resultPanel = this,
            ...
            allScenario = [],
            set, index2;
        for (set = 0; set < possibleSet; set++) {
            if (numFail == resultPanel.popCnt(set)) {
                var scenario = [];
                for (index2 = 0; index2 < rateLen; index2++) {
                    if (set & (1 << index2)) {
                        // Fail
                        scenario.push(100 - itemBoostFn(rateList[index2]))
                    } else {
                        // Success
                        scenario.push(itemBoostFn(rateList[index2]))
                    }
                }
    ...
    // Produce stats table
    getSpecInfo: function(affixList) {
        var resultPanel = this,
            abStore = resultPanel.abilitySet.abilityStore,
            affixEntry, affixEffect, affixBreakdown, overallStat = {},
            text = [],
            regex = new RegExp("([^\\(]+)\\(([\\+\\-]\\d+)\\)"),
            affixLen = affixList.length,
            outputText = "",
            index;
        for (index = 0; index < affixLen; index++) {
            affixEntry = abStore.findRecord("name", affixList[index].name) || resultPanel.optionStore2.findRecord("name", affixList[index].name);
            affixEffect = affixEntry.get("effect").replace(/<br>/g, ",").split(",");
            for (j = 0; j < affixEffect.length; j++) {
                affixBreakdown = affixEffect[j].match(regex);
                if (affixBreakdown && affixBreakdown.length == 3) {
                    ...
                    if (affixBreakdown === null) {
                        text.push(affixEffect[j])
                    }
                }
            }
        }
        // Output stat info
        for (index = 0; index < resultPanel.abText.length; index++) {
    ...
    getSuccessTable: function(affixList, col, colHead) {
        var resultPanel = this,
            rateList = [],
            potBoostFn = resultPanel.getSelectOptionRecord(resultPanel.selOpt3).get("fn"),
            len = affixList.length,
            optItem1List = resultPanel.selOpt1.store,
            opt1Len = optItem1List.count(),
            rate, index;
        for (index = 0; index < len; index++) {
            rate = resultPanel.calcSameBonus(affixList[index], resultPanel.chkOpt1.getSameCount());
        ...
        for (index = 0; index < opt1Len; index++) {
    ...
    getSuccessGraph: function(affixList, col, colHead) {
        var len = affixList.length;
        return {
            xtype: "chart",
    ...
    getOrderView: function(affixList) {
        var resultPanel = this,
            len = affixList.length,
            htmlAffixList = [],
            fieldset, index;
        ...
        if (affixGroup.name == "A") {
            ...
            if (affixGroup.name == "A+") {
                ...
                if (affixGroup.name == "A++") {
                    ...
                    if (affixGroup.name == "B") {
                        return "Resistance"
                    } else {
                        if (affixGroup.name == "C") {
                            return "Status Effect"
                        } else {
                            if (affixGroup.name == "D") {
                                return "Soul"
                            } else {
                                if (affixGroup.name == "D+") {
                                    return "Fever"
                                } else {
                                    if (affixGroup.name == "E") {
                                        return "Special"
                                    } else {
                                        if (affixGroup.name == "L") {
                                            return "Super Special Abilities"
                                        }else {
										   if (affixGroup.name == "LOSS") {
